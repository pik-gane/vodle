...
| 6 — Matrix hardening & completion | **in progress** | Started 2026-09-09: `scripts/test-matrix.sh` (throw-away Synapse harness, port 8009) and `src/app/matrix-two-client.spec.ts` (real two-client convergence: alias discovery, cross-client ratings, offline reconvergence, fresh-session authority), wired into CI. Found & fixed: `MatrixService.register()` never worked against a standard Synapse (no user-interactive-auth handling — every registration got a 401). Findings to address next: the offline queue (`enqueueOfflineEvent`) has **no internal callers**, so writes while offline are lost rather than queued; `environment.matrix.enable_e2ee` is decorative (no crypto backend is initialized); the room-alias domain is derived from the homeserver URL's hostname and silently breaks when it differs from the server_name. Remaining: two-homeserver federation test, migration tooling validation, E2EE, perf/security report. |
