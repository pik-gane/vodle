...
- The write pace is `matrix.writes_per_second` in the environment (20, the same figure as the recommended `rc_message.per_second`), so a deployment that raises its homeserver's limits is not held back by its client instead, and an account the homeserver does not rate-limit at all can set 0 and send as fast as it likes. A refusal from any server starts the spacing again whatever the setting.
