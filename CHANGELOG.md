...
- Delegation is enabled (`environment.delegation.enabled`). It had been off in both environments since the Matrix backend was written; the machinery — the request and response events, the magic link to the delegate, the weight limit and the tally — has been in place and covered by specs since [#333](https://github.com/pik-gane/vodle/issues/333). What it exposes is written down in `documentation/PRIVACY.md` §6: the delegation details are encrypted under the poll password, but the delegation id is part of the (unencrypted) event type of the record at each end, so the homeserver can read the delegation graph by voter id, and the delegate's own user room names the poll. The second of those adds nothing the user room did not already say; both are fixed by the same change, §8.
