...
#
# Mirrors scripts/test-couchdb.sh otherwise: the servers are configured like
# the development homeserver from docker-compose.yml / INSTALL.md — open
# registration without verification, rate limits effectively disabled — and
# the guard-bot account is registered on hs1 so poll-room creation can invite
...
register_user() {  # register_user CLIENT_PORT NAME PASSWORD  (idempotent)
  local port="$1" name="$2" password="$3" response
  response=$(curl -sS -X POST "http://127.0.0.1:${port}/_matrix/client/v3/register" \
    -H 'Content-Type: application/json' \
    -d "{\"username\":\"${name}\",\"password\":\"${password}\",\"auth\":{\"type\":\"m.login.dummy\"}}")
  case "${response}" in
    *access_token*) echo "registered ${name} on port ${port}" ;;
    *M_USER_IN_USE*) echo "${name} already registered on port ${port}" ;;
...
start() {
  ...
  # the guard bot lives on hs1; hs2's rooms invite it across federation
  register_user 8009 "${GUARD_BOT_USER}" "${GUARD_BOT_PW}"
  start_guard_bot
